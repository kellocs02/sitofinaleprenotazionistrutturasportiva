  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'Georgia', serif" }} className="flex min-h-screen">
        ...
        {/* ── RIGHT PANEL — Bianco panna ── */}
        <div className="w-full lg:w-1/2 bg-[#f5f0e8] flex flex-col justify-between px-14 py-14">
          ...
          <div className={`transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <p className="text-[#1a3a2a]/40 text-xs uppercase tracking-[0.2em] mb-3">Bentornato</p>
            ...
            {locked ? (
              <div className="lock-pulse max-w-xs rounded-xl border border-[#1a3a2a]/30 bg-[#1a3a2a]/10 p-6 text-center space-y-3">
                ...
                <p className="text-[#1a3a2a] font-semibold text-sm uppercase tracking-wide">Account temporaneamente bloccato</p>
                <p className="text-[#0d1f15]/50 text-xs leading-relaxed">
                  Troppi tentativi falliti.<br />Riprova tra:
                </p>
                <div className="text-4xl font-bold text-[#1a3a2a] tracking-wider">
              ...
              <form onSubmit={handleSubmit} className="space-y-5 max-w-xs">
                  ...
                  <div className="flex gap-1.5 items-center">
                    {Array.from({ length: MAX_ATTEMPTS }).map((_, i) => (
                      <div key={i} className={`h-0.5 flex-1 rounded-full transition-all duration-300
                        ${i < attempts ? 'bg-[#1a3a2a]' : 'bg-[#1a3a2a]/15'}`} />
                    ))}
                    <span className="text-[11px] text-[#0d1f15]/30 ml-1 whitespace-nowrap">
                ...
                <div>
                  <label className="block text-[11px] font-semibold tracking-[1.5px] uppercase text-[#0d1f15]/40 mb-1.5">Email</label>
                  <input
                    type="email"
                    name="email"
                    ...
                    className="w-full bg-white/60 border border-[#1a3a2a]/15 rounded-lg px-4 py-3 text-[#0d1f15] text-sm
                      placeholder:text-[#0d1f15]/25 outline-none transition-all duration-200
                      focus:border-[#1a3a2a]/50 focus:bg-white focus:shadow-[0_0_0_3px_rgba(26,58,42,0.08)]"
                    style={{ fontFamily: "'Georgia', serif" }}
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-semibold tracking-[1.5px] uppercase text-[#0d1f15]/40 mb-1.5">Password</label>
                  <input
                    type="password"
                    name="password"
                ...
                {error && (
                  <div className="flex items-center gap-2 bg-[#1a3a2a]/8 border border-[#1a3a2a]/20 text-[#1a3a2a] px-4 py-3 rounded-lg text-[13px]">
                    <span>⚠️</span> {error}
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#1a3a2a] text-[#f5f0e8] font-semibold text-sm py-3.5 rounded-lg
                    transition-all duration-200 hover:enabled:bg-[#254d38] hover:enabled:-translate-y-px
                    hover:enabled:shadow-[0_6px_20px_rgba(26,58,42,0.25)]
          ...
          <div className="space-y-1.5">
            <p className="text-[#0d1f15]/40 text-xs">
              Non hai un account?{' '}
              <Link href="/registrazione" className="text-[#1a3a2a] font-semibold hover:text-[#254d38] transition-colors">
                Registrati gratis
              </Link>
            </p>
            <p>
              <a href="/recupera-password" className="text-[#0d1f15]/30 text-xs hover:text-[#1a3a2a] transition-colors">
                Password dimenticata?
              </a>
            </p>
            <p className="text-[#0d1f15]/20 text-xs pt-2">© 2025 Campo Sportivo Panoramica</p>
          </div>
